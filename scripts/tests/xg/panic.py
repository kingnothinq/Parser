...
def test(device):
    ...
    def find_panic(panic):
        ...
        if 'LINKDOWN' in panic:
            results.append('Watchdog activation detected: XG_TIMEOUT. '
            ...
            results.append('Watchdog activation detected: UD. '
                          'There was a failure in the radio modem and the device was rebooted. '
                          'If this watchdog works too often, then contact the developers.')

        elif 'SIN' in panic:
            results.append('Watchdog activation detected: SIN. '
                          'There was a failure in the radio modem and the device was rebooted. '
                          'If this watchdog works too often, then contact the developers.')

        elif 'ASSERT' in panic:
            results.append('Assert was detected. '
                          'If this assert works too often, then contact the developers.')

        else:
            results.append(f'An unknown panic message was found. '
    ...
    if results:
        ...
        return ('Panic and asserts', results)
    else:
