...
def analyze(dc_name, dc):
    ...
    try:
        ...
        if search(r'#\sR5000\sWANFleX\sH09', dc_string) is not None:
            ...
            report = dc_reporter.create_report(iwmux, dc_tester.run_tests(iwmux), dc_name)
        #dc_reporter.write_report(report, iwmux.serial_number)
        #dc_reporter.debug_report(report)
        ...
        #R5000 series
            ...
            report = dc_reporter.create_report(r5000, dc_tester.run_tests(r5000), dc_name)
            #dc_reporter.write_report(report, r5000.serial_number)
            #dc_reporter.debug_report(report)

        #XG series
        elif search(r'#\sXG\sWANFleX\sH12', dc_string) is not None:
            xg = dc_parser.parse_xg(dc_string, dc_list)
            report = dc_reporter.create_report(xg, dc_tester.run_tests(xg), dc_name)
            #dc_reporter.write_report(report, xg.serial_number)
            #dc_reporter.debug_report(report)

        #Quanta series
        elif search(r'#\sOCTOPUS-PTP\sWANFleX\sH18', dc_string) is not None:
            quanta = dc_parser.parse_quanta(dc_string, dc_list)
            report = dc_reporter.create_report(quanta, dc_tester.run_tests(quanta), dc_name)
            #dc_reporter.write_report(report, quanta.serial_number)
            #dc_reporter.debug_report(report)
        else:
            raise
    ...
    #"""
        ...
        report = dc_reporter.error_report(dc_name)
        #dc_reporter.debug_report(report)
    finally:
        return report
    #"""
